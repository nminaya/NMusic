...
public class EditarRolView extends JFrame{
	...
	private void setRol(){
		nombreRol.setText(rol.getNombreRol());
		for (int i = 0; i < rol.getPrivilegios().length; i++) {
			for (int j = 0; j < privilegiosChbx.length; j++) {
				if(privilegiosChbx[j].getText().equals(rol.getPrivilegios()[i].getNombrePrivilegio())){
					privilegiosChbx[j].setSelected(true);
				}
			}
		}
	}
	...
	private void saveRol() {
		...
		if (isValidForm) {
			...
			try {
				DataAccess.InterfaceDAL.getInstance().updateRol(rol);
				Helper.showMessage("Rol Actualizado");
				this.dispose();
			} catch (Exception e) {
